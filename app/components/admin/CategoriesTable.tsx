        ...
        <div className="flex gap-2 justify-end">
          ...
          <button
            onClick={async () => {
              ...
              try {
                await deleteCategoryAction(categoryId); // ✅ Server Action
  ...
  return (
    <div className="bg-eshop-bgCard overflow-hidden mb-16">
      ...
      <div className="md:hidden grid gap-4 p-4">
        {paginatedCategories.map((category) => (
          <div key={category.id} className="bg-eshop-formsBackground border border-eshop-textSecondary rounded-lg p-5 hover:border-eshop-goldDeep/50">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-black text-eshop-textPrimary text-lg truncate uppercase tracking-wide">{category.titulo}</h3>
                <code className="text-[10px] text-eshop-goldDeep font-bold uppercase">Slug: {category.slug}</code>
              </div>
            </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-6 py-4 bg-eshop-formsBackground border-t border-eshop-textSecondary">
          ...
          <span className="text-sm font-semibold text-eshop-textPrimary">{currentPage} / {totalPages}</span>
          <button
      ...
      <div className="px-6 py-4 bg-eshop-formsBackground/50 border-t border-eshop-textSecondary">
        <p className="text-xs font-bold text-eshop-textSecondary uppercase tracking-widest text-center md:text-left">
          Total: <span className="text-eshop-goldDeep">{categories.length}</span>{' '}
