...
export default function PaymentCallbackPage() {
  ...
  useEffect(() => {
    ...
    const checkPaymentStatus = async () => {
      ...
      try {
        // 🔎 Verificar pago con tu backend
        const response = await fetch(`/api/verify-payment?id=${id}`);
        const data = await response.json();

        if (data.status === "APPROVED") {
          // 🔥 Recuperar datos guardados antes de ir a Wompi
          const pendingOrderRaw = localStorage.getItem("pendingOrder");

          if (!pendingOrderRaw) {
            setStatus("error");
            setMessage(
              "No se encontró información del pedido. Contacta soporte."
            );
            return;
          }
          ...
          if (!saveResult.success) {
            console.error("Error guardando orden:", saveResult);
            setStatus("error");
            setMessage("Error guardando el pedido. Contacta soporte.");
            return;
          }
          ...
          setOrderReference(
            ref || data.data?.reference || saveResult.reference || ""
          );

          toast.success(
            "¡Compra realizada con éxito! Tu pedido será procesado pronto.",
            {
              duration: 5000,
              icon: "🎉",
            }
